...
Drafts only. Nothing here has been sent, and nothing here gets sent without
Owen doing it himself.
...
Two things forced the merge. Draft 02 was addressed to OSF's "Request Access"
control, and that control does not exist here: the API reports
`access_requests_enabled: true`, but the node is public and has zero child
components, so OSF renders no button. Email is the only route. And with both
...
No address was found for Mohammadhamed Shadbeh or Khashayar Khajavi. Shadbeh is
the thesis author and therefore the most likely person to actually hold the
code, so 01 asks Wang to forward rather than trying to guess a student address.
