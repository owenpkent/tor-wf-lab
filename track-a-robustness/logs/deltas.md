...
| 2026-09-06 | Holmes attribution, unattributable classes | `assert >= 12` per class | Classes that cannot supply 2 background plus 1 attributed sample are skipped, and take the median effective range over the classes that could | The post-Conflux AU collection has a class with 10 traces in total, so its validation share is 1. The effective range only sets the truncation window used to augment that class, and the median over the other 110 classes is a better estimate than any fixed constant. Which classes were skipped, and which were attributed over fewer samples, is recorded per row as `attr_coverage` in runs made after this change, and replayed for the runs committed before it in `holmes-attr-coverage.md`. |

| 2026-09-06 | Vantage confound | Table 4.1 trains AU and tests across networks; Table 4.3 trains UK and tests across time | Added `vantage-au` and `vantage-uk`: one training collection, one anchor, one label space, with both network cells and the drift cell scored against it, run from each vantage in turn | The thesis's two axes do not share a training collection, and AU-trained models degrade about 2.9x more in general, so vantage and axis are entangled in its comparison and in ours. These axes remove the confound, and running the same test from both vantages showed the answer it gives is itself vantage-dependent: AU-trained, 2 of 5 classifiers are network-limited; UK-trained, 0 of 4. Only the ordering by network sensitivity is invariant. An addition, not a replacement: the main table still follows the thesis's own design so the numbers stay traceable to theirs. Results in `results/vantage/` and `results/vantage-uk/`. Holmes was not run on the UK vantage, which is recorded in the verdict. |
